...
(map! :leader
    :desc "Kill buffer" "x" #'iflipb-kill-buffer
...
Add paths to treesitter grammar libraries:
#+begin_src emacs-lisp :tangle yes
...
* Wayland
This dynamically sets the ~WAYLAND_DISPLAY~ variable, which is needed
to launch GUI applications.
In practice, this is only needed when using the daemon.

We register our function as a hook for each new emacs frame.

#+begin_src emacs-lisp :tangle yes
(defun ll/set-wayland-socket ()
  (unless (getenv "WAYLAND_DISPLAY")
    (let* ((runtime (or (getenv "XDG_RUNTIME_DIR")
                        (format "/run/user/%s" (user-uid))))
           (wayland (car (directory-files runtime nil "^wayland-[0-9]+$"))))
      (when wayland
        (setenv "WAYLAND_DISPLAY" wayland)))))

(add-hook! 'server-after-make-frame-hook :append #'ll/set-wayland-socket)
#+end_src
...
Use [[https://github.com/emacs-citar/citar?tab=readme-ov-file#usage][citar]] to auto complete, store notes, and manage a ~bibtex~ bibliographical collection.

#+begin_src emacs-lisp :tangle yes
(use-package! citar
  :config
  (setq citar-bibliography (directory-files-recursively "/home/laurent/org/bibliography/bibs" (concat "\\" ".bib" "$")))
  (setq citar-library-paths '("/home/laurent/org/bibliography/files/")
        citar-notes-paths '("/home/laurent/org/bibliography/notes/"))
...
~citar~ is an interface to ~org-cite~ that allows to (among other things):

- Open the ~pdf~ file that correspond to a bibliograph entry (if it exists).
   The file must be located at ~citar-library-paths~ and
  named ~${citekey}.pdf~ to be detected.
- Create an org-roam note using a pre-defined template.
  Here, we generate an ~org-roam~ entry (file) in ~citar-notes-paths~.
- Open the URL of the paper (if it exists).

*** Defining template
Here, we define a number of templates to add a note for each paper, namely:
- ~extended~: This will create a note in a separate file. This
  is the default behaviour when selecting the /Create Org-Roam Notes/ function
  in the main ~citar~ menu.
- ~short~: This will capture a note and append it to an existing org file.
  This boils down to generating a heading and appending it to an existing file.
  This behaviour can be accessed using the custom interactive function defined
  in a next section.

#+begin_src emacs-lisp :tangle yes
(setq org-roam-capture-templates
        '(("s" "short bibliography note" entry "* ${citar-title}\n:PROPERTIES:\n:ID: %(org-id-new)\n:CITEKEY: ${citar-citekey}\n:AUTHOR: ${citar-author}\n:DATE: ${citar-date}\n:END:\n%?"
           :target (file "/home/laurent/org/bibliography/inbox.org")
           :prepend t
           :unnarrowed t
           )
          ("e" "extended bibliography note (separate file)" plain "%?"
           :if-new (file+head "/home/laurent/org/bibliography/notes/${citar-citekey}.org"
                              "#+title: Notes on ${title}#+SETUPFILE: /home/laurent/org/bibliography/setup_file.org\n%?* References :ignore:\n#+print_bibliography:")
           :unnarrowed t)
          ))
#+end_src

We set the "extended" template by default.
#+begin_src emacs-lisp :tangle yes
(setq citar-org-roam-capture-template-key "e")
#+end_src

*** Note taking using interactive template selection

We define an intermediate hydra to select a template prior to creating a note.

#+begin_src emacs-lisp :tangle yes
(defhydra ll/citar-template-hydra (:color blue :hint nil)
  "
Select a template:

_s_: short note
_e_: extended note (separate file)
_q_: quit
"
  ("s"
   (let ((citar-org-roam-capture-template-key "s"))
     (call-interactively #'citar-create-note))
   "short")

  ("e"
   (let ((citar-org-roam-capture-template-key "e"))
     (call-interactively #'citar-create-note))
   "extended")

  ("q" nil "quit"))

(defun ll/citar-create-note ()
  "Prompt for a citar template via Hydra and create a note."
  (interactive)
  (ll/citar-template-hydra/body))
#+end_src

*** Interactively open bibliography file
#+begin_src emacs-lisp :tangle yes
(defun ll/open-citar-bibliography ()
  "Select and open a file from citar-bibliography using completing-read."
  (interactive)
  (if (boundp 'citar-bibliography)
      (if citar-bibliography
          (let ((selected-file (completing-read "Select file: " citar-bibliography)))
            (if (and selected-file (not (string-empty-p selected-file)))
                (find-file selected-file)
              (message "No file selected")))
        (error "citar-bibliography is empty"))
    (error "citar-bibliography is not defined")))
#+end_src

*** Set destination where captured notes can go
#+begin_src emacs-lisp :tangle yes
(setq org-refile-targets `(
(,(directory-files "~/org/bibliography/" t "\\.org$") :maxlevel . 2)))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(map! :leader
      :prefix-map ("n b" . "bibliography") 
      :desc "open main" "m" 'citar-open
      :desc "open entry" "e" 'citar-open-entry
      :desc "create note" "c" #'ll/citar-create-note
      :desc "open link" "l" #'citar-open-links
      :desc "open file" "f" #'citar-open-files
      :desc "open reference file" "r" #'ll/open-citar-bibliography
      )
    ;; (map! :leader
    ;;     :mode notmuch-mode
...
#+begin_src emacs-lisp :tangle yes
(setq ll/notmuch-default-query "tag:inbox AND not tag:deleted AND not tag:sent AND date:3months..")
(after! notmuch
    ...
    (map! :leader
        ...
        :map notmuch-mode-map
        :desc "Saved searches" "mj" 'notmuch-jump-search
        :desc "View part" "mv" 'notmuch-show-view-part
        ))
#+end_src
...
Increase refresh rate. Default is ~0.1s~
#+begin_src emacs-lisp :tangle yes
(use-package! vterm
  :custom
